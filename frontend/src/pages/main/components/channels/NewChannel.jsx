...
const NewChannel = ({ channel }) => {
  ...
  const handleRenameChannel = (currentChannel) => {
    dispatch(
      showModal({ type: modalTypes.RENAMING, data: { ...currentChannel } })
    );
  };
    ...
    <li className='nav-item w-100'>
      <Dropdown as={ButtonGroup} className='d-flex'>
        <Button
          ...
          variant={active}
          className={`w-100 rounded-0 text-start text-truncate ${active}`}
        >
        ...
        <Dropdown.Toggle
          variant={active}
          className={`flex-grow-0 ${active}`}
          split
        >
          <span className='visually-hidden'>
            {t('pageChat.channels.channelControl')}
          </span>
        </Dropdown.Toggle>
        <Dropdown.Menu>
          <Dropdown.Item
            ...
            eventKey='1'
          >
            {t('pageChat.channels.removeChannelButton')}
          </Dropdown.Item>
          <Dropdown.Item
            onClick={() => handleRenameChannel(channel)}
            eventKey='2'
          >
            {t('pageChat.channels.renameChannelButton')}
          </Dropdown.Item>
        </Dropdown.Menu>
      </Dropdown>
    </li>
